...
export default (
  ...
  const handleDropdownInputChange = ({ target: { value, name } }: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleCountNameChange = ({ target: { value } }: React.ChangeEvent<HTMLInputElement>) => {
    setCountName(value)
  }
