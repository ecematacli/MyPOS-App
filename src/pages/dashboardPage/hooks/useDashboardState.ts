...
export default () => {
  ...
  const makeApiCall = async (url: string, name: string = '', method: string = 'get') => {
    try {
  ...
  const fetchRevenueData = async (displayOption: string, start: Date, end: Date) => {
    let option: string
    ...
    const url = getRequestParams(`/stats/revenue-chart?option=${option}`, start, end)
  ...
  const fetchTopSellingProducts = async (pageNumber: number = 1, start: Date, end: Date) => {
  ...
  const fetchLastActivities = async () => {
    const data: LastActivitiesData = await makeApiCall('/events/sales', 'activities')
