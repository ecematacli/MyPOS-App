...
export default (batchId: string) => {
  ...
  const fetchBatchesProducts = async (id: string, status = 'all', page = 1, rowsPerPage = 10) => {
    try {
      setLoading(true)
  ...
  const handleTabsChange = (e: React.ChangeEvent<HTMLInputElement>, newValue: string) => {
    ...
    setPage(1)
    fetchBatchesProducts(batchId, newValue, 1, rowsPerPage)
  }
  ...
  const replaceBatchProduct = (idToReplace: string, replacement: BatchProduct) => {
    setBatchProducts(batchProducts => ({
      ...batchProducts,
      products: batchProducts.products.map(product => (product.id === idToReplace ? replacement : product)),
    }))
  }
  ...
  const handleChangeRowsPerPage = ({ target: { value } }: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fetchBatchesProducts(batchId, tabsValue, page, numValue)
  }

  const handleChangePage = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, newPage: number) => {
    //To adapt 0-based page of MUI pagination component 1 is added whilst 1 is subtracted for page prop
    if (newPage + 1 < 0) return
    setPage(newPage + 1)
    fetchBatchesProducts(batchId, tabsValue, newPage + 1, rowsPerPage)
  }
  ...
  const searchProducts = async (query: string): Promise<BatchProduct[]> => {
    try {
      const { data } = await api.get(`inventory-count/${batchId}/search-products?query=${query}`)
      return data
    } catch (e) {
