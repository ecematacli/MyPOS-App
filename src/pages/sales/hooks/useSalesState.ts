...
const productsReducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case ActionTypes.EditProductField: {
      return state.map(p =>
        p.id === action.payload.id ? { ...p, [action.payload.field]: action.payload.newValue } : p
      )
    }
...
export default (storage?: any) => {
  ...
  const [products, dispatch] = useLocalStorageReducerState('products', [], productsReducer, storage)
  ...
  const editProductField = (id: string, field: string, newValue: number) => {
    dispatch({
