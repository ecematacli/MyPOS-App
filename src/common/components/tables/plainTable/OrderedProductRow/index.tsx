...
const OrderedProductRow: React.FC<Props> = ({ row }) => {
  ...
  const { id, name, barcode, sku, variation, brand, category, discountPrice, price, qty, synced } = row
  ...
  return (
    <TableRow hover>
      ...
      <TableCell style={{ width: '8%' }}>{category ? category.name : '-'}</TableCell>
        ...
        <Align align='center'>
          ...
          {!synced && (
            <IconButton size='small'>
              <Refresh onClick={() => reSync({ id, type: 'stockOrder', products: [row] })} />
            </IconButton>
          )}
        </Align>
      </TableCell>
