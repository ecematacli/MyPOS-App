...
import { TableContainer, Table, TableBody, TableCell, TableHead, TableRow, TablePagination } from '@material-ui/core'
...
const CustomTable: React.FC<TableProps> = ({
  ...
  const renderExpandIconContainer = (id: string) => (
    <div className={classes.iconContainer}>
      {expandedRows[id] ? <ExpandLess className={classes.icon} /> : <ExpandMore className={classes.icon} />}
    </div>
  )
  ...
  const renderNoDisplay = () => (
    <TableRow>
      <TableCell className={classes.noDisplayCell} colSpan={10}>
        <div className={classes.noDisplayMsg}>{`No ${tableType === 'sales' ? 'sales' : 'products'} to display`}</div>
      </TableCell>
    </TableRow>
  )
      ...
      <div className={classes.tableContainer}>
        <Table className={classes.table}>
          {tableHeads !== undefined ? (
            <TableHead>
              <TableRow className={classes.tableHeadRow}>{renderTableHead()}</TableRow>
            </TableHead>
          ) : null}
          <TableBody>{!count ? renderNoDisplay() : renderTableBody()}</TableBody>
        </Table>
      </div>
